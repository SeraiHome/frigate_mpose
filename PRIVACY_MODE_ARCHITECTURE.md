...
### Background modes (`privacy_background`)

| Value | Background | Source | Notes |
|---|---|---|---|
| `black` (default) | Solid black canvas | N/A | Obvious privacy indicator, zero cost |
| `scene` | Grayscale static room | `motion_detector.avg_frame` | Reuses the motion detector's running weighted average of the scene. No extra compute — the motion detector already maintains this. The image is at reduced `motion_frame_size` resolution and upscaled, giving a slightly blurred look that reinforces "this is not real footage". Converges within seconds of startup. |

The background image flows through the existing rendering pipeline:
- `render_skeleton_bgr()` accepts an optional `bg_image` parameter
- If provided, it resizes to frame dimensions, converts grayscale→BGR, and uses it as the canvas
- Colored skeleton lines and keypoint dots are drawn on top
- The result is converted to YUV I420 for SHM

For the privacy proxy sidecar, the same `scene` background can be achieved by building an `avg_frame` from `cam_real` using the same `cv2.accumulateWeighted` approach, or by receiving the background via MQTT as a retained JPEG message.
...
privacy_background: str = Field(
    default="black",
    title="Background style: 'black' (solid) or 'scene' (motion detector avg_frame).",
)
...
cameras:
  living_room:
    ...
    pose:
      ...
      privacy_background: scene  # NEW: "black" or "scene" (room background from motion detector)
...
|---|---|---|---|
| **Frigate web UI** | Skeleton on black/scene (SHM substitution) | Real RGB (override active) | Real RGB |
| **Frigate recordings** | Skeleton on black/scene (from `cam_priv`) | Real RGB (proxy pass-through) | Real RGB |
| **Frigate snapshots** | Skeleton on black/scene (from SHM) | Real RGB | Real RGB |
| **LiveKit video call** | Skeleton on black/scene (edge reads `cam_priv`) | Real RGB (proxy switches) | Real RGB |
| **Birdseye composite** | Skeleton on black/scene (reads SHM) | Real RGB | Real RGB |
| **Home Assistant** | Skeleton on black/scene (via Frigate integration) | Real RGB | Real RGB |
